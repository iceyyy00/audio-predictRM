...
MODEL_PATH = 'trained_model_reverb.keras'
EFFECT_CLASSES_FILE = 'effect_classes_reverb.txt'

print("Context: pendeteksian jenis reverb pada audio.")
...
if os.path.exists(EFFECT_CLASSES_FILE):
    ...
    EFFECT_CLASSES = [
        'Acoustic', 'Ambience', 'Artificial', 'Bathroom', 'Big-Cave', 'Cathedral',
        'Church-Hall', 'Clean', 'Dance-Vocal', 'Large-Room', 'Medium-Room',
        'Modern-Vocal', 'Modern', 'Small-Room-Bright', 'Small-Room-Dark',
        'Vocal-I', 'Vocal-II', 'Voice-Tail'
    ]
...
def predict_audio(file_path, threshold=0.5):
    try:
        ...
        print("✓ Detected reverb types:", effects)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        dataset_path = os.path.join('dataset_reverb')
        audio_files = []
        ...
        for f in audio_files[:5]:  # Test 5 file pertama
            predict_audio(f)